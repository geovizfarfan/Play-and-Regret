...
async function runGame(channel, players, bet, sizeKey, guild, mode) {
  ...
  if (emojis.length < size.pairs) {
    return channel.send(`❌ Not enough server emojis! Need at least **${size.pairs}** static emojis. Add more to the server first.`);
  }
  ...
  const makeEmbed = (status = '') => {
    const scoreStr = mode === 'multi'
      ...
      : `🎯 **${moves}** moves${streak >= 2 ? ` • 🔥 **${streak} streak!**` : ''}`;

    return new EmbedBuilder()
      .setColor('#9B59B6')
      .setTitle(`🧠 Memory Game${mode === 'multi' ? ' — Multiplayer' : ` — ${players[0].username}`}`)
      .setDescription(
  ...
  collector.on('collect', async (interaction) => {
    ...
    if (mode === 'multi' && interaction.user.id !== currentPlayer().id) {
      return interaction.reply({ content: `⚠️ It's **${currentPlayer().username}'s** turn!`, ephemeral: true });
    }
    if (mode === 'solo' && interaction.user.id !== players[0].id) {
      return interaction.reply({ content: `⚠️ This isn't your game!`, ephemeral: true });
    }
    ...
    if (firstPick === null) {
      ...
      if (board[first].emojiId === board[idx].emojiId) {
        ...
        let bonusMsg = '';
        if (mode === 'solo') {
          if (streak === 3)     { bonusEarned += Math.floor(bet * 0.25); bonusMsg = `🔥 3 streak! +${Math.floor(bet * 0.25)} bonus!`; }
          else if (streak === 5){ bonusEarned += Math.floor(bet * 0.50); bonusMsg = `🔥🔥 5 streak! +${Math.floor(bet * 0.50)} bonus!`; }
          else if (streak >= 7) { bonusEarned += Math.floor(bet);        bonusMsg = `🔥🔥🔥 ${streak} streak! +${Math.floor(bet)} bonus!`; }
        }
        ...
        await gameMsg.edit({
          embeds: [makeEmbed(`✅ **Match!**${bonusMsg ? ' ' + bonusMsg : ''} ${mode === 'multi' ? currentPlayer().username + ' goes again!' : ''}`)],
          components: buildComponents(board, gameId, size.cols),
        }).catch(() => {});
        ...
        await gameMsg.edit({
          embeds: [makeEmbed(`❌ No match!${mode === 'multi' ? ` **${currentPlayer().username}'s** turn!` : ''}`)],
          components: buildComponents(board, gameId, size.cols),
        }).catch(() => {});
      }
    }
        ...
        if (totalMatched === size.pairs) {
          ...
          await channel.send({ embeds: [
            new EmbedBuilder().setColor('#FFD700')
              ...
              .setDescription(
                `🧠 **${players[0].username}** solved it in **${fmtTime(elapsed)}** with **${moves}** moves!\n\n` +
                `<a:moneybag:1479268556687540345> Won: **${total.toLocaleString()} oops**` +
                (bonusEarned > 0 ? ` *(+${bonusEarned} streak bonus)*` : '') + `\n` +
                `<a:jackpot:1479203793806557385> **${Math.floor(bet * 0.10)} oops** → jackpot`
              )
          ]});
        } else {
...
async function launchMemory(channel, bet, sizeKey, mode, triggeredBy, hostId, guild) {
  ...
  if (activeGames.has(channelId)) return channel.send('❌ There\'s already a Memory Game running here!');
  ...
  async handleSlash(interaction, commandName) {
    if (commandName === 'memory') {
      ...
      try {
        ...
        await interaction.editReply({ content: `🧠 Memory Game started!` });
      } catch (err) {
      ...
      await interaction.editReply({ embeds: [
        new EmbedBuilder()
          .setColor('#9B59B6')
          .setTitle(`🧠 Memory Leaderboard — ${size.label}`)
