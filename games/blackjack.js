...
function streakMsg(userId) {
  ...
  if (s >= 7)  return `🔥🔥🔥 **${s} WIN STREAK!** Double bonus!`;
  if (s >= 5)  return `🔥🔥 **${s} win streak!** +50% bonus!`;
  if (s >= 3)  return `🔥 **${s} win streak!** +25% bonus!`;
  return null;
}
...
function makeSignupEmbed(players, bet, timeLabel, mode = 'multi') {
  return new EmbedBuilder()
    ...
    .setDescription(
      ...
      `<a:moneybag:1479268556687540345> Bet: **${bet} oops** per player\n` +
...
const makeButtons = (mode = 'multi') => new ActionRowBuilder().addComponents(
  new ButtonBuilder().setCustomId('bj_join_placeholder').setLabel('🃏 Join Blackjack').setStyle(ButtonStyle.Primary),
...
function makeGameEmbed(dealerHand, playerHands, hideSecond = true) {
  ...
  for (const [uid, data] of playerHands) {
    ...
    const status = data.stand ? '✋ Stand' : data.bust ? '💥 Bust' : data.blackjack ? '🃏 Blackjack!' : `${total}`;
    fields.push({
      name: `${data.username}`,
...
async function runBlackjack(channel, players, bet) {
  ...
  if (dealerIsAce) {
    ...
    const insMsg = await channel.send({
      content: `⚠️ **Dealer shows an Ace!** Side bet: pay half your bet (${Math.floor(bet/2)} oops) for **Insurance**. If dealer has Blackjack, insurance pays 2:1.`,
    ...
    insCollector.on('collect', async (interaction) => {
      ...
      if (!pd || pd.insurance) {
        return interaction.reply({ content: `⚠️ Already insured or not in game.`, ephemeral: true });
      }
      const bal = await economy.getBalance(interaction.user.id);
      const ins = Math.floor(bet / 2);
      if (bal < ins) return interaction.reply({ content: `❌ Not enough oops for insurance!`, ephemeral: true });
      await economy.removeFunds(interaction.user.id, ins, 'BJ Insurance');
      pd.insurance = true;
      await interaction.reply({ content: `🛡️ **${interaction.user.username}** took insurance for **${ins} oops**.`, ephemeral: false });
    });
  ...
  for (const [uid, pd] of playerHands) {
    ...
    const turnMsg = await channel.send({
      content: `🃏 <@${uid}>'s turn! Your hand: **${fmtHand(pd.hand)}** (${handTotal(pd.hand)})\nYou have 30 seconds — **Hit** or **Stand**?`,
      components: [new ActionRowBuilder().addComponents(
      ...
      col.on('collect', async (interaction) => {
        ...
        if (interaction.customId === hitId) {
          ...
          if (total > 21) {
            ...
            pd.stand = true;
            await channel.send(`✅ **${pd.username}** hit 21! Hand: **${fmtHand(pd.hand)}**`);
            col.stop('21');
          } else {
  ...
  if (dealerIsAce && dealerBJ) {
    for (const [uid, pd] of playerHands) {
      if (pd.insurance) {
        ...
        await channel.send(`🛡️ **${pd.username}** insurance pays out **${payout} oops**!`);
      }
    }
  }
  ...
  for (const [uid, pd] of playerHands) {
    ...
    if (won && payout > 0) {
      ...
      results.push(
        ...
        `Hand: ${fmtHand(pd.hand)} (${pTotal}) → **+${finalPayout.toLocaleString()} oops**` +
...
async function launchBlackjack(channel, bet, triggeredBy, hostId, mode = 'multi', signupSecs = 30) {
  ...
  if (activeGames.has(channelId)) return channel.send('❌ There\'s already a Blackjack game running here!');
  ...
  const makeButtons = (m = 'multi') => new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(joinId).setLabel('🃏 Join Blackjack').setStyle(ButtonStyle.Primary),
  ...
  if (mode === 'solo') {
    ...
    if (bal < bet) return channel.send(`❌ **${triggeredBy}** needs **${bet} oops** to play!`);
    await economy.removeFunds(hostId, bet, 'Blackjack solo entry');
    game.players.push({ id: hostId, username: triggeredBy });
    game.phase = 'running';
    await channel.send({ embeds: [new EmbedBuilder().setColor('#1A1A2E').setTitle('🃏 BLACKJACK — Solo Game').setDescription(`**${triggeredBy}** vs the Dealer\n\n<a:moneybag:1479268556687540345> Bet: **${bet} oops**`)] });
  ...
  collector.on('collect', async (interaction) => {
    ...
    if (interaction.customId === startId) {
      if (interaction.user.id !== g.hostId) {
        return interaction.reply({ content: `❌ Only **${g.triggeredBy}** can force start!`, ephemeral: true });
      }
    ...
    if (g.players.find(p => p.id === interaction.user.id)) {
      return interaction.followUp({ content: `⚠️ You're already in!`, ephemeral: true });
    }
    await economy.getUser(interaction.user.id, interaction.user.username);
    const bal = await economy.getBalance(interaction.user.id);
    if (bal < bet) return interaction.followUp({ content: `❌ You need **${bet} oops** to join!`, ephemeral: true });
  ...
  collector.on('end', async (_, reason) => {
    ...
    const closed = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(joinId).setLabel('🃏 Table Closed').setStyle(ButtonStyle.Secondary).setDisabled(true),
    ...
    if (g.players.length < 1) {
      activeGames.delete(channelId);
      return channel.send(`❌ Blackjack cancelled — nobody joined.`);
    }

    g.phase = 'running';
    await channel.send(`🃏 **Blackjack starting!** ${g.players.map(p => `**${p.username}**`).join(', ')} vs the Dealer!`);
...
module.exports = {
  ...
  async handleSlash(interaction, commandName) {
    if (commandName === 'blackjack') {
      ...
      await interaction.reply({ content: `🃏 Starting Blackjack...`, ephemeral: true });
      ...
      if (!g) return interaction.reply({ content: `❌ No Blackjack running here.`, ephemeral: true });
      if (g.phase === 'running') return interaction.reply({ content: `❌ Game is in progress.`, ephemeral: true });
      ...
      await interaction.reply({ content: `✅ Blackjack cancelled. **${g.players.length}** player(s) refunded.` });
    }
  },
  ...
  async handleCommand(message, args, command) {
    if (command === 'blackjack' || command === 'bj') {
      ...
      if (!g) return message.reply(`❌ No Blackjack running here.`);
      for (const p of g.players) await economy.addFunds(p.id, g.bet, 'Blackjack cancelled');
      if (g.message) g.message.edit({ components: [] }).catch(() => {});
      activeGames.delete(message.channel.id);
      return message.reply(`✅ Blackjack cancelled. **${g.players.length}** player(s) refunded.`);
    }
  },
};
