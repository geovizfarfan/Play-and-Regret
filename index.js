...
const GAME_RULES = {
  ...
  stickers: () => `👻 **Drop It Like It's Hot — How to Play**\nJust chat! Enough real conversation in an allowed channel randomly spawns a collectible sticker with a CATCH button. First click opens a 5-second window — anyone can jump in, then one random winner takes it. Higher rarities are rarer and stick around longer before escaping.\n\nUse \`/stickers book\` to see your collection, \`/stickers missing\` for what you don't have, \`/stickers trade\` to swap 1-for-1 with someone, and \`/stickers exchange\` to turn duplicates into sins.`,
};
