/**
 * utils/shop.js
 * PostgreSQL version — same public API as before.
 */
...
const shop = {
  ...
  // init() is now a no-op — tables created in initDB()
  async init() {},
  ...
  async hasToken(userId, tokenId) {
    ...
    const row = await db.get(
      'SELECT id FROM shop_inventory WHERE user_id = ? AND token_id = ?',
      [userId, tokenId]
    );
    return !!row;
  },
  ...
  async buyToken(userId, tokenId) {
    await db.run(
      'INSERT INTO shop_inventory (user_id, token_id) VALUES (?, ?) ON CONFLICT DO NOTHING',
      [userId, tokenId]
    );
  },
  ...
  async equip(userId, tokenId) {
    ...
    if (!token) return false;
    await db.run(
      `INSERT INTO shop_equipped (user_id, slot, token_id) VALUES (?, ?, ?)
       ON CONFLICT (user_id, slot) DO UPDATE SET token_id = EXCLUDED.token_id`,
      [userId, token.slot, tokenId]
    );
    return true;
  },
  ...
  async getEquipped(userId, slot) {
    ...
    const row = await db.get(
      'SELECT token_id FROM shop_equipped WHERE user_id = ? AND slot = ?',
      [userId, lookupSlot]
    );
    if (!row) {
  ...
  async setItem(userId, itemType, emoji) {
    await db.run(
      `INSERT INTO shop_items (user_id, item_type, emoji, equipped) VALUES (?, ?, ?, 1)
       ON CONFLICT (user_id, item_type) DO UPDATE SET emoji = EXCLUDED.emoji`,
