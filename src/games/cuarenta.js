...
async function startGame(channel, authorId, authorUsername, is2v2, bet, replyFn, vsBot=false) {
  ...
  const lobbyMsg = await channel.send({ embeds: [new EmbedBuilder().setColor('#C9B1FF')
    ...
    components: [new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('cq_join').setLabel('Join').setEmoji('<a:ecuadorflag:1511506710152613998>').setStyle(ButtonStyle.Secondary),
...
async function handleInstantCuarenta(channel, game, pIdx) {
  ...
  await channel.send({ embeds: [new EmbedBuilder().setColor('#F2F5E0').setTitle('<:373840:1511531231756419202> CUARENTA! Instant Win!')
...
async function postTurn(channel, game) {
  ...
  const msg = await channel.send({ embeds: [new EmbedBuilder()
    ...
    .setDescription(
      `<@${player.id}> — your turn!\n\n` +
      `**<a:cards:1511530261551124561> Table (${game.table.length}):** ${cardLinks(game.table)}\n\n` +
      `Press **<a:eyes:1511507447704191026> My Hand** to see your cards and play.`
    )
...
async function handleTableBtn(inter, g) {
  ...
  await inter.reply({ embeds: [new EmbedBuilder().setColor('#D8EDF0').setTitle('<a:cards:1511530261551124561> Table Cards')
...
async function handleRondaBtn(inter, g, channel) {
  ...
  p.rondaClaimed = true;
  p.score += 4;
  await inter.reply({content:`<a:Loading:1511506718666784778> Ronda claimed! **+4 pts** added.`, ephemeral:true});
  await channel.send({ embeds: [new EmbedBuilder().setColor('#E0F0E0').setTitle('<a:Loading:1511506718666784778> ¡Ronda!')
    .setDescription(`**${p.username}** calls Ronda! *(3 of the same hidden rank)*\n**+4 pts** · rank is **secret**\n\n<a:Warning:1497476844860215366> Player to their left earns +10 pts if they capture a Ronda card with a Caída!\n\n${scoreBar(g)}`)] });
}
...
async function handleCuarentaBtn(inter, g, channel, col, onPlayed=()=>{}) {
  ...
  await inter.reply({content:`<:373840:1511531231756419202> Cuarenta confirmed!`, ephemeral:true});
  await channel.send({ embeds: [new EmbedBuilder().setColor('#F2F5E0').setTitle('<:373840:1511531231756419202> CUARENTA! Instant Win!')
    .setThumbnail(cardURL(four[0]))
...
async function handleHandBtn(inter, g, channel, col, onPlayed=()=>{}) {
  ...
  await inter.reply({ embeds: [new EmbedBuilder()
    ...
    .setTitle(isMyTurn?'<a:eyes:1511507447704191026> Your Hand — choose a card':'<a:eyes:1511507447704191026> Your Hand')
  ...
  playCol.on('collect', async ci => {
    ...
    await ci.update({ content:'<:checkmark:1495666088417956002> Playing card...', components:[] }).catch(() => {});
    col.stop('played');
...
async function resolvePlay(channel, game, cardIdx) {
  ...
  await channel.send({ embeds: [new EmbedBuilder().setColor(color)
    ...
    .setDescription(desc)
    .addFields(
      { name:`<a:cards:1511530261551124561> Table (${game.table.length})`, value: cardLinks(game.table) },
      { name:'<a:marked:1511508970882465832> Scores', value: scoreBar(game) },
    )
...
async function resolveHandScoring(channel, game) {
  ...
  await channel.send({ embeds: [new EmbedBuilder().setColor('#D8EDF0').setTitle('<a:marked:1511508970882465832> Hand Over — Card Count Scoring')
...
async function doBotTurn(channel, game) {
  ...
  if (special?.type === 'cuarenta' && !bot.cuarentaClaimed) {
    ...
    await channel.send({ embeds: [new EmbedBuilder().setColor('#F2F5E0').setTitle('<:373840:1511531231756419202> CUARENTA! Bot Wins!')
  ...
  if (special?.type === 'ronda' && !bot.rondaClaimed) {
    ...
    await channel.send({ embeds: [new EmbedBuilder().setColor('#E0F0E0').setTitle('<a:Loading:1511506718666784778> Bot claims Ronda!')
...
module.exports = {
  ...
  async handleSlash(interaction, commandName) {
    if (commandName === 'cuarenta') {
      ...
      const bet   = feeEnabled ? (interaction.options.getInteger('bet') || 100) : 0;
  ...
  async handleCommand(message, args, command) {
    if (command === 'cuarenta') {
      ...
      const bet   = feeEnabled ? (parseInt(args.find(a=>!isNaN(parseInt(a)))) || 100) : 0;
  ...
  getRulesText() {
    return [
      ...
      `**<a:Loading:1511506718666784778> Ronda:** 3 of same rank after deal → press Ronda button → +4pts (rank secret). Opponent earns +10pts if they Caída a Ronda card.\n`,
      `**<:373840:1511531231756419202> Cuarenta:** 4 of same rank → press Cuarenta button → instant win!\n`,
      `**<a:Warning:1497476844860215366> At 38pts:** You can ONLY win with a pure Caída (+2pts = exactly 40). Limpia doesn't score.\n`,
      `**<a:marked:1511508970882465832> Card scoring** (end of each deal): 20 cards=6pts, each extra pair=+2pts. Under 20: most cards=2pts.\n`,
      `**<a:trophies:1507765453299122387> Win:** Reach exactly **40 points**. Overshooting caps at 39.`,
    ].join('\n');
  },
