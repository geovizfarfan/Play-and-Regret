...
async function launchBigBag(channel, totalAmount, droppedBy) {
  ...
  collector.on('end', async (_, reason) => {
    ...
    if (reason === 'time' && remaining > 0) {
      ...
      await channel.send(
        `<a:583778moneyfly:1479271753392853023> **Drop Expired** Nobody grabbed the remaining **${remaining.toLocaleString()}** <a:SINS:1522338223613804724>. ${bagQuip} <a:583778moneyfly:1479271753392853023>`
      );
    }
  });
}
...
async function launchQuickDrop(channel, amount, dropper) {
  ...
  const makeEmbed = (done = false, claimedBy = null, quip = null) => new EmbedBuilder()
    ...
    .setDescription(done
      ? (claimedBy
        ? `<a:congrats:1478999022072238222> **Drop CLAIMED!**\n**${claimedBy}** snatched **${amount.toLocaleString()}** <a:SINS:1522338223613804724> from <@${dropper.id}>! <a:moneybag:1479268556687540345>`
      ...
      : `<a:SINS:1522338223613804724> **sins DROP!** <@${dropper.id}> dropped **${amount.toLocaleString()} sins**! <a:moneybag:1479268556687540345>\n\nFirst to press the button claims it all! <a:run:1479270296140910653>\n\n⏳ Drop expires in 2 minutes`)
  ...
  collector.on('collect', async (interaction) => {
    ...
    await interaction.followUp({
      content: `<a:congrats:1478999022072238222> **Drop CLAIMED!** **${interaction.user.username}** snatched **${amount.toLocaleString()}** <a:SINS:1522338223613804724> from ${dropper.username}! <a:moneybag:1479268556687540345>`,
    });
