...
module.exports = {
  ...
  async placeBet(message, args) {
    ...
    if (amount < 10) return message.reply(`${E.ERROR} Minimum bet is 10 sins!`);
    ...
    if (economy.getBalance(message.author.id) < amount) return message.reply(`${E.ERROR} You don't have enough sins!`);
    ...
    return message.reply({ embeds: [
      new EmbedBuilder()
        ...
        .setTitle(`Bet Placed!`)
        .setDescription(`**${message.author.username}** bet **${amount} sins** on **${sideEmoji} ${side.toUpperCase()}** for:\n*${bet.title}*`)
        .addFields(
          { name: `${E.BET_YES} YES pool`,        value: `${totals.yes} sins`,                             inline: true },
          { name: `${E.BET_NO} NO pool`,           value: `${totals.no} sins`,                              inline: true },
          { name: `${E.BB_COIN} Total Pool`,       value: `${totals.total} sins`,                           inline: true },
          { name: '📈 Potential payout',           value: `~${this.calcPayout(amount, side, totals)} sins`, inline: true }
        )
    ]});
  },
  ...
  async getBetTotalsAsync(betId) {
    const entries = await db.all('SELECT side, SUM(amount) as total FROM bet_entries WHERE bet_id = ? GROUP BY side', [betId]);
    const yes = entries.find(e => e.side === 'yes')?.total || 0;
    const no  = entries.find(e => e.side === 'no')?.total  || 0;
    return { yes, no, total: yes + no };
  },
  ...
  async listBets(message) {
    ...
    for (const bet of bets) {
      ...
      embed.addFields({
        ...
        value: `Pool: **${totals.total} sins** | ${E.BET_YES} YES: ${totals.yes} sins | ${E.BET_NO} NO: ${totals.no} sins | ${E.CLOCK} ${timeLeft}h left\n\`!bet ${bet.id} yes/no <amount>\``
      });
    }
  ...
  async betInfo(message, args) {
    ...
    const yesBettors = entries.filter(e => e.side === 'yes').map(e => `${e.username}: ${e.amount} sins`).join('\n') || 'None';
    const noBettors  = entries.filter(e => e.side === 'no').map(e => `${e.username}: ${e.amount} sins`).join('\n')  || 'None';
    ...
    return message.reply({ embeds: [
      new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: `${E.BB_COIN} Pool`,   value: `${totals.total} sins`,     inline: true },
          { name: `${E.BET_YES} YES`,    value: `${totals.yes} sins`,        inline: true },
          { name: `${E.BET_NO} NO`,      value: `${totals.no} sins`,         inline: true },
  ...
  async resolveBet(message, args) {
    ...
    const totals  = await this.getBetTotalsAsync(betId);
    ...
    await db.run('UPDATE bets SET status = "resolved", outcome = ? WHERE id = ?', [outcome, betId]);
    ...
    for (const entry of winEntries) {
      ...
      payoutLines.push(`${entry.username}: +${payout} sins`);
    }
    ...
    return message.channel.send({ embeds: [
      new EmbedBuilder()
        ...
        .setFooter({ text: `Total pot: ${totals.total} sins` })
    ]});
  },
  ...
  async myBets(message) {
    ...
    const lines = entries.map(e => {
      ...
      return `**#${e.bet_id}** ${e.title.substring(0, 40)} — **${e.side.toUpperCase()}** for **${e.amount} sins**${result}`;
    });
  ...
  async createBetDirect(interaction, title, desc, hours) {
    ...
    await interaction.editReply({ content: `✅ Bet **"${title}"** created! (#${betId})`, ephemeral: true });
    ...
    collector.on('collect', async (i) => {
      ...
      if (i.customId === cancelId) {
        if (i.user.id !== interaction.user.id && !i.member?.permissions.has('Administrator')) {
          return i.reply({ content: `❌ Only the bet creator or an admin can cancel this bet!`, ephemeral: true });
        }
        ...
        await i.channel.send(`🚫 **Bet #${betId} "${title}"** has been cancelled. ${entries.length} player(s) refunded.`);
      ...
      if (bal < 10) return i.reply({ content: '❌ You need at least 10 sins to bet!', ephemeral: true });
    ...
    interaction.client.on('interactionCreate', async (i) => {
      ...
      if (!match) return;
      const [, id, side, amount] = match;
      if (parseInt(id) !== betId) return;

      await i.deferReply({ ephemeral: true }).catch(() => {});

      const bal = await economy.getBalance(i.user.id);
      if (bal < parseInt(amount)) {
        return i.editReply({ content: `❌ You need **${amount} sins** but only have **${bal.toLocaleString()}**!` });
      }

      const bet = await db.get('SELECT * FROM bets WHERE id = ?', [betId]);
      if (!bet || bet.status !== 'open') {
        return i.editReply({ content: `❌ This bet is no longer open!` });
      }

      const existing = await db.get('SELECT * FROM bet_entries WHERE bet_id = ? AND user_id = ?', [betId, i.user.id]);
      if (existing) {
        return i.editReply({ content: `⚠️ You already placed a bet on this one!` });
      }

      await economy.removeFunds(i.user.id, parseInt(amount), `Bet #${betId}`);
      await db.run(
        'INSERT INTO bet_entries (bet_id, user_id, username, side, amount) VALUES (?, ?, ?, ?, ?)',
        [betId, i.user.id, i.user.username, side, parseInt(amount)]
      );
      await db.run('UPDATE bets SET total_pool = total_pool + ? WHERE id = ?', [parseInt(amount), betId]);

      // Update embed with live pool
      const updatedBet = await db.get('SELECT total_pool FROM bets WHERE id = ?', [betId]);
      const yesTotal   = await db.get('SELECT SUM(amount) as t FROM bet_entries WHERE bet_id = ? AND side = "yes"', [betId]);
      const noTotal    = await db.get('SELECT SUM(amount) as t FROM bet_entries WHERE bet_id = ? AND side = "no"', [betId]);
      const updatedEmbed = new EmbedBuilder()
        ...
        .setDescription(
          ...
          `✅ YES: **${(yesTotal?.t || 0).toLocaleString()} sins** • ❌ NO: **${(noTotal?.t || 0).toLocaleString()} sins**`
        )
        .addFields(
          { name: `${E.CLOCK} Closes In`, value: `${hours} hours`, inline: true },
          { name: '🆔 Bet ID',            value: `#${betId}`,      inline: true },
      ...
      await i.editReply({
        content: `${sideEmoji} **${i.user.username}** bet **${amount} sins** on **${side.toUpperCase()}** for bet #${betId}!`,
      });
    });
  },
