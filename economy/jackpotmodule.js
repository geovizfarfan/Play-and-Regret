...
const CURRENCY = 'oops';
const POT_NAME = '💰 oops I am Rich Pot';
...
const STREAK_BONUSES = [
  { min: 2,  bonus: 50,   label: '🔥 2-Win Streak Bonus!'     },
  { min: 3,  bonus: 100,  label: '🔥🔥 3-Win Streak Bonus!'   },
...
function buildJoinButton(sessionId, disabled = false) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`oops_rich_join:${sessionId}`)
      .setLabel('🎟️ Enter the oops I am Rich Pot!')
  ...
  const menu = new StringSelectMenuBuilder()
    ...
    .addOptions(sessions.map(s => {
      ...
      return new StringSelectMenuOptionBuilder()
        .setLabel(label)
        .setDescription(`Pot: ${s.pot.toLocaleString()} oops • Draws in: ${timeLeft}`)
...
module.exports = {
  ...
  async handleSelect(interaction) {
    ...
    if (action === 'richpot_live') {
      ...
      return interaction.editReply(`✅ Live display pinned in <#${channel.id}>!`);
    }
  },
  ...
  async handleModal(interaction) {
    if (interaction.customId.startsWith('oops_rich_modal:')) {
      const sessionId = parseInt(interaction.customId.split(':')[1]);
      const raw       = interaction.fields.getTextInputValue('oops_rich_number');
    ...
    return replyFn({ embeds: [
      new EmbedBuilder()
        ...
        .setDescription(
          ...
          `It's like a lottery — one lucky player walks away rich. 💰`
        )
        .addFields(
          { name: '🎯 Your Number',    value: `**${number}**`,                          inline: true },
          { name: `${E.BB_COIN} Pot`, value: `**${pot.toLocaleString()} ${CURRENCY}**`, inline: true },
          { name: '👥 Entries',        value: `**${entries.length}** players`,           inline: true },
          { name: '⏳ Draw In',         value: timeLeft,                                 inline: true },
        )
  ...
  async adminStart(message, args) {
    ...
    const name   = args.slice(0, -1).join(' ') || 'oops I am Rich Pot';
    const mode   = args[args.length - 1] || 'weekly';
    return this._startJackpot(name, mode, message.channel, null, async (msg) => message.channel.send(msg));
  },
  async adminStartSlash(interaction) {
    ...
    await interaction.deferReply({ ephemeral: true });
    const name    = interaction.options.getString('name') || 'oops I am Rich Pot';
    ...
    return this._startJackpot(name, mode, channel, roleId, async () => {});
  },

  async _startJackpot(name, mode, channel, roleId, replyFn) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**What is this?**\n` +
        `It's a jackpot lottery! Pay **${jackpot.ENTRY_COST} oops** to enter and pick a number 1-100.\n` +
        `When time is up, we draw a random number — whoever's pick is **closest wins the entire pot!** 🎉\n\n` +
        `💸 Game ties (game draws feed into the pot automatically)\n\n` +
        `⏳ Draw in: **${label}** · Ends: <t:${Math.floor(endsAt.getTime()/1000)}:F>`
      )
      .addFields(
        { name: '💡 How to Win', value: 'Pick the closest number to the draw — you win the whole pot!', inline: false },
      )
      .setFooter({ text: `Entry: ${jackpot.ENTRY_COST} oops • Click the button to enter!` });


    const ping = roleId ? `<@&${roleId}>` : '';
    await channel.send({
      content: ping ? `${ping} <a:jackpot:1479203793806557385> **${name} is now LIVE!**` : undefined,
      embeds: [embed],
      components: [buildJoinButton(session.id)],
    });
    await this.updateLiveChannels();
    this._scheduleAutoDraw(session.id, ms, channel);
    return replyFn(`✅ **${name}** started!`);
  },
  ...
  async _runDraw(channel, sessionId, replyFn) {
    ...
    const embed = new EmbedBuilder()
      .setColor('#FFE4A0')
      .setTitle(`💰 ${name} — DRAW RESULTS!`)
      .setDescription(
        `🎲 The winning number was: **${result.winningNum}**\n\n` +
        `${E.TROPHY || '🏆'} **<@${result.winner.user_id}> wins the pot!**\n` +
        `Their pick **#${result.winner.number}** was the closest!\n\n` +
        `${E.BB_COIN} **${result.pot.toLocaleString()} ${CURRENCY}** added to their balance!` +
        (result.tied ? `\n\n*(Tiebreaker applied)*` : '')
      )
      .addFields(
        { name: '🎯 Winning #',      value: `**${result.winningNum}**`,                       inline: true },
        { name: '🏆 Winner',         value: `<@${result.winner.user_id}>`,                    inline: true },
        { name: '📍 Their Pick',     value: `**${result.winner.number}**`,                    inline: true },
        { name: `${E.BB_COIN} Won`, value: `**${result.pot.toLocaleString()} ${CURRENCY}**`, inline: true },
        { name: '👥 Entries',        value: `**${result.entries.length}**`,                   inline: true },
      )
      .setFooter({ text: 'Start the next pot with /jackpotstart!' });

    const msg = { content: `<@${result.winner.user_id}> 💰 You just got oops I am Rich!`, embeds: [embed] };
    await this.updateLiveChannels();
    return replyFn ? replyFn(msg) : channel.send(msg);
  },
  ...
  async _showEntriesForSession(sessionId, isAdmin, replyFn) {
    ...
    const entries = await jackpot.getEntries(sessionId);
    if (!entries.length)
      return replyFn({ embeds: [new EmbedBuilder().setColor('#D4E8E8')
        .setTitle(`💰 ${session?.name} — Participants`)
        .setDescription('No entries yet! Be the first 🎟️')
        .addFields({ name: `${E.BB_COIN} Pot`, value: `**${session?.pot?.toLocaleString() || 0} oops**`, inline: true })]});
    ...
    return replyFn({ embeds: [
      new EmbedBuilder()
        ...
        .addFields(
          { name: `${E.BB_COIN} Pot`,  value: `**${session?.pot?.toLocaleString() || 0} oops**`, inline: true },
  ...
  async _showHistory(replyFn) {
    ...
    if (!rows.length) return replyFn('💰 No draws yet!');
    const lines = rows.map(r =>
      `**${r.session_name || 'Pot'}** (${r.drawn_at?.slice(0,10)}) — 🎯 #${r.winning_num} → <@${r.winner_id}> won **${r.amount_won?.toLocaleString()} ${CURRENCY}**`
    ).join('\n');
    return replyFn({ embeds: [
      new EmbedBuilder().setColor('#D9B3FF').setTitle('💰 oops I am Rich Pot — History')
  ...
  async setLiveChannel(message, args) {
    ...
    if (sessions.length === 1) {
      ...
      return message.reply(`✅ Live display for **${sessions[0].name}** pinned here!`);
    }
  ...
  async setLiveChannelSlash(interaction) {
    ...
    if (sessions.length === 1) {
      ...
      return interaction.editReply(`✅ Live display for **${sessions[0].name}** pinned in <#${channel.id}>!`);
    }
  ...
  async awardStreakBonus(userId, username, streak, channel) {
    ...
    if (channel) await channel.send({ embeds: [
      new EmbedBuilder().setColor('#FFD4A8').setTitle(tier.label)
        .setDescription(`<@${userId}> is on a **${streak}-win streak** in Tic-Tac-Bruh!\n${E.BB_COIN} **+${tier.bonus} oops** streak bonus!`)
        .setFooter({ text: STREAK_BONUSES.map(t => `${t.min} wins = +${t.bonus} oops`).join(' • ') })
    ]});
  },
  ...
  async _stopAndRefund(sessionId, replyFn) {
    ...
    const drawFundNote = drawFundRestored > 0
      ? `\n💸 **${drawFundRestored.toLocaleString()} oops** from game draws saved to the draw fund for the next pot.`
      : '';
    ...
    return replyFn({ embeds: [
      new EmbedBuilder()
        ...
        .setDescription(
          refunded.length > 0
            ? `**${refunded.length} player${refunded.length !== 1 ? 's' : ''}** refunded **${jackpot.ENTRY_COST} oops** each.` + drawFundNote
            : `No entries to refund.` + drawFundNote
        )
        .addFields(
          { name: '👥 Players Refunded', value: `**${refunded.length}**`,                                             inline: true },
          { name: '💰 Total Refunded',   value: `**${(refunded.length * jackpot.ENTRY_COST).toLocaleString()} oops**`, inline: true },
        )
...
function buildSessionEmbed(session, entries) {
  ...
  const color    = pot >= 5000 ? '#FFE4A0' : pot >= 1000 ? '#D9B3FF' : '#B3D4FF';
  const name     = session?.name || POT_NAME;

  // Visual pot fill bar (10 blocks, fills as pot grows toward 10,000 oops)
  const filled = Math.min(Math.round((pot / 10000) * 10), 10);
  const potBar = '🟨'.repeat(filled) + '⬛'.repeat(10 - filled);

  const embed = new EmbedBuilder()
    .setColor(color)
    .setTitle(active ? `💰 ${name} — LIVE!` : `💰 ${name} — Ended`)
    ...
    .addFields(
      { name: `${E.BB_COIN || '💰'} Current Pot`, value: `**${pot.toLocaleString()} oops**`, inline: true },
      { name: '👥 Players Entered',                value: `**${entries.length}**`,            inline: true },
      { name: '⏳ Draw In',                         value: timeLeft ? `**${timeLeft}**` : '—', inline: true },
      { name: '📊 Pot Level',                       value: potBar,                             inline: false },
    )
    .setTimestamp()
...
async function buildOverviewPayload() {
  ...
  const embeds = sessions.map(s => {
    const color    = s.pot >= 5000 ? '#FFE4A0' : s.pot >= 1000 ? '#D9B3FF' : '#B3D4FF';
    const timeLeft = formatTimeLeft(new Date(s.ends_at) - Date.now());
    return new EmbedBuilder()
      .setColor(color)
      .setTitle(`💰 ${s.name} — LIVE!`)
      .addFields(
        { name: `${E.BB_COIN || '💰'} Pot`, value: `**${s.pot.toLocaleString()} oops**`, inline: true },
        { name: '⏳ Draws In',               value: timeLeft,                             inline: true },
      )
      .setFooter({ text: `Pot ID: ${s.id} • Entry: ${jackpot.ENTRY_COST} oops` });
  });
