 ...
 *
 * 🎒 Big Bag (admin only)
 *   Admin throws a bag of X oops. Anyone who clicks gets a random slice.
 ...
 *
 * 💸 Quick Drop (anyone)
 *   User drops X oops. First person to click claims the whole thing.
...
const EXPIRED_QUIPS = [
  ...
  'Nobody showed up. The oops felt personally attacked.',
...
async function launchBigBag(channel, totalAmount, droppedBy) {
  ...
  const makeEmbed = (remaining, claimers, done = false) => {
    ...
    return new EmbedBuilder()
      ...
      .setDescription(done
        ? `<:OOPS:1478993005187698789> **Big Bag — Closed!**\n\n${claimers > 0 ? `**${claimers} member${claimers !== 1 ? 's' : ''}** grabbed from the bag.` : 'Nobody grabbed anything.'}\n${remaining > 0 ? `<a:583778moneyfly:1479271753392853023> **${remaining.toLocaleString()} oops** unclaimed → <a:jackpot:1479203793806557385>` : '✅ Bag fully emptied!'}`
        : `<:OOPS:1478993005187698789> **Big Bag Drop!** **${droppedBy}** threw a bag of oops! <a:moneybag:1479268556687540345>\n\nClick fast — each grab gets a random slice! <a:run:1479270296140910653>\n\n<a:moneybag:1479268556687540345> **${remaining.toLocaleString()} oops** remaining\n${bar} ${pct}% claimed\n\n<:countdown:1479295748884529215> Expires in 2 mins`)
      .addFields(
        { name: '<a:moneybag:1479268556687540345> Total Bag',  value: `${totalAmount.toLocaleString()} oops`, inline: true },
  ...
  collector.on('collect', async (interaction) => {
    ...
    await interaction.followUp({
      content: `🎒 **${interaction.user.username}** grabbed **${actual.toLocaleString()} oops** from the bag!`,
    });
  ...
  collector.on('end', async (_, reason) => {
    ...
    if (reason === 'time' && remaining > 0) {
      ...
      await channel.send(
        `<a:583778moneyfly:1479271753392853023> **Drop Expired** Nobody grabbed the remaining **${remaining.toLocaleString()}** <:OOPS:1478993005187698789>. ${bagQuip} <a:583778moneyfly:1479271753392853023>`
      );
    }
  });
}
...
async function launchQuickDrop(channel, amount, dropper) {
  ...
  const makeEmbed = (done = false, claimedBy = null, quip = null) => new EmbedBuilder()
    ...
    .setDescription(done
      ? (claimedBy
        ? `<a:congrats:1478999022072238222> **Drop CLAIMED!**\n**${claimedBy}** snatched **${amount.toLocaleString()}** <:OOPS:1478993005187698789> from <@${dropper.id}>! <a:moneybag:1479268556687540345>`
        : `<a:583778moneyfly:1479271753392853023> **Drop Expired**\nNobody claimed the **${amount.toLocaleString()}** <:OOPS:1478993005187698789> in time. ${quip || 'It vanished into thin air!'} <a:583778moneyfly:1479271753392853023>`)
      : `<:OOPS:1478993005187698789> **oops DROP!** <@${dropper.id}> dropped **${amount.toLocaleString()} oops**! <a:moneybag:1479268556687540345>\n\nFirst to press the button claims it all! <a:run:1479270296140910653>\n\n⏳ Drop expires in 2 minutes`)
  ...
  collector.on('collect', async (interaction) => {
    ...
    await interaction.followUp({
      content: `<a:congrats:1478999022072238222> **Drop CLAIMED!** **${interaction.user.username}** snatched **${amount.toLocaleString()}** <:OOPS:1478993005187698789> from ${dropper.username}! <a:moneybag:1479268556687540345>`,
    });
  ...
  collector.on('end', async (_, reason) => {
    ...
    await channel.send(
      `<a:583778moneyfly:1479271753392853023> **Drop Expired** Nobody claimed the **${amount.toLocaleString()}** <:OOPS:1478993005187698789> in time. ${quip} <a:583778moneyfly:1479271753392853023>`
    );
  });
}
...
module.exports = {
    ...
    if (commandName === 'bigbag') {
      try {
        ...
        if (!hasHostRole(interaction.member)) {
          return interaction.editReply(`❌ Only admins and **${EVENT_HOST_ROLE}** can throw a Big Bag!`);
        }
        const amount = interaction.options.getInteger('amount');
        await economy.getUser(interaction.user.id, interaction.user.username);
        const bal = await economy.getBalance(interaction.user.id);
        if (bal < amount) return interaction.editReply(`❌ You need **${amount.toLocaleString()} oops** but only have **${bal.toLocaleString()}**!`);
        await economy.removeFunds(interaction.user.id, amount, 'Big Bag drop');
        await interaction.editReply(`✅ Big Bag of **${amount.toLocaleString()} oops** thrown!`);
      ...
      await interaction.editReply(`✅ You dropped **${amount.toLocaleString()} oops**!`);
  ...
  async handleCommand(message, args, command) {
    ...
    if (command === 'bigbag') {
      if (!hasHostRole(message.member)) return message.reply(`❌ Only admins and **${EVENT_HOST_ROLE}** can throw a Big Bag!`);
      await economy.getUser(message.author.id, message.author.username);
      const bal = await economy.getBalance(message.author.id);
      if (bal < amount) return message.reply(`❌ You need **${amount.toLocaleString()} oops** but only have **${bal.toLocaleString()}**!`);
