...
module.exports = {
  ...
  async _processBuyToken(user, tokenId, replyFn) {
    ...
    return replyFn({ embeds: [
      new EmbedBuilder()
        .setColor('#B3FFD9')
        .setTitle(`<:oops:1478993005187698789> Purchase Successful!`)
      ...
      components.push(new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder()
          ...
          .addOptions(pieces.slice(0, 25).map(t => ({
            ...
            description: t.price === 0 ? 'Free default' : `${t.price} oops`,
...
async function buildShopDisplay(user) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `<:oops:1478993005187698789> **Balance: ${bal.toLocaleString()} oops**\n` +
  ...
  if (pieces500.length) {
    components.push(new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        ...
        .setPlaceholder('🎮 Piece Tokens — 500 oops (Non-Animated)')
        .addOptions(pieces500.slice(0, 25).map(t => ({
          label: t.name,
          value: t.id,
          emoji: parseEmoji(t.emoji),
          description: `500 oops • works as X or O piece`,
        })))
    ));
  }
  ...
  if (pieces1000.length) {
    components.push(new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        ...
        .setPlaceholder('🎮 Piece Tokens — 1,000 oops (Animated)')
        .addOptions(pieces1000.slice(0, 25).map(t => ({
          label: t.name,
          value: t.id,
          emoji: parseEmoji(t.emoji),
          description: `1,000 oops • works as X or O piece`,
        })))
    ));
  }
